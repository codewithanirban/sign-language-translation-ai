...
class RealTimeSignLanguageAnalyzer:
    ...
    def stop_camera(self):
        ...
        # If recording still active, stop it and finalize files
        if self.recording:
            # Call toggle_recording to finalize files properly
    ...
    def toggle_recording(self):
        ...
        if not self.recording:
            ...
            # Initialize video writer (write annotated frames resized to 640x480)
    ...
    def _annotate_frame(self, frame, hand_landmarks, predicted_class, confidence):
        """
        Annotate frame with:
         - MediaPipe landmarks (mp_drawing)
         - Additional small circles at landmark points for clearer marking
         - Prediction text and confidence
        Returns annotated frame (BGR).
        """
        annotated = frame.copy()
        h, w, _ = annotated.shape

        # Draw MediaPipe default landmarks & connections
        if hand_landmarks is not None:
            self.mp_drawing.draw_landmarks(annotated, hand_landmarks, self.mp_hands.HAND_CONNECTIONS)

            # Also draw small circles on each landmark for clearer video visibility
            for lm in hand_landmarks.landmark:
                x_px, y_px = int(lm.x * w), int(lm.y * h)
                # small filled circle
                cv2.circle(annotated, (x_px, y_px), 3, (0, 255, 255), -1)  # cyan-ish marker

        # Put prediction text at top-left
        if predicted_class is not None:
            txt = f"Pred: {predicted_class}  Conf: {confidence:.3f}"
            # background rectangle for readability
            (text_w, text_h), baseline = cv2.getTextSize(txt, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)
            cv2.rectangle(annotated, (10, 10), (10 + text_w + 10, 10 + text_h + 10), (0, 0, 0), -1)
            cv2.putText(annotated, txt, (15, 10 + text_h), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        ...
        while self.is_running:
            if self.cap and self.cap.isOpened():
                ...
                if ret:
                    ...
                    # Make prediction if we have keypoints & model
                    if keypoints is not None and self.current_model is not None:
                        try:
                            prediction, confidence = self.predict(keypoints)
                            if prediction is not None:
                                predicted_class = self.class_names[prediction]
                        except Exception as e:
                            # Keep running even if prediction fails
                            print("Prediction error:", e)
                            predicted_class = None
                            confidence = 0.0

                    # If recording: prepare annotated frame and write annotated video
                    annotated_for_save = None
                    if self.recording:
                        # Annotate with landmarks and text BEFORE resizing/writing
                        annotated_for_save = self._annotate_frame(frame, hand_landmarks, predicted_class, confidence)
                        # resize to 640x480 for writer consistency
                        annotated_resized = cv2.resize(annotated_for_save, (640, 480))
                        if self.video_writer:
                            self.video_writer.write(annotated_resized)

                        # Save per-frame keypoints + prediction into JSON array
                        # If no keypoints, store None
                        kp_list = keypoints.tolist() if keypoints is not None else None
                    ...
                    # Draw landmarks and prediction text on frame that will be shown in GUI
                    # (So GUI and saved video match)
                    display_frame = self._annotate_frame(frame, hand_landmarks, predicted_class, confidence)

                    # If prediction is available, update GUI labels and store test_results as before
                    if predicted_class is not None:
                        # Update prediction display on GUI
                        self.root.after(0, lambda pc=predicted_class: self.pred_label.config(text=f"Prediction: {pc}"))
                        self.root.after(0, lambda c=confidence: self.conf_label.config(text=f"Confidence: {c:.3f}"))
                        
                        # Record into test_results list (keeps original behavior)
                        if self.recording:
                    ...
                    # Convert display_frame (BGR) to RGB and show in tkinter
                    ...
                    # Update video label
                    self.root.after(0, lambda ft=frame_tk: self.video_label.config(image=ft))
    ...
    def on_closing(self):
        ...
        # Ensure recording and writer are finalized
        if self.recording:
            self.toggle_recording()
        self.stop_camera()
        self.root.destroy()
